            ...
            if (snapshot.val()) {
              ...
              for (var index = 0; index < users.length; index++) {
                if (!Expo.isExpoPushToken(users[index].pushtoken)) {
                  console.error(
                    `Push token ${users[index]
                      .pushtoken} is not a valid Expo push token`
                  );
                  continue;
                }
                messages.push({
              ...
              var promises = chunks.map(function(chunk) {
                return expo.sendPushNotificationsAsync(chunk).then(receipts => {
            ...
            .once("value", function(snapshot) {
              if (snapshot.val()) {
                ...
                if (!Expo.isExpoPushToken(targetedTradePushToken)) {
                  console.error(
                    `Push token ${targetedTradePushToken} is not a valid Expo push token`
                  );
                  res
                    .status(500)
                    .send(
                ...
                messages.push({
                  ...
                  sound: "default",
                  body: `${req.user
                    .displayName} asked you for the reserved coffee. Do you accept the trade?`,
                  data: {
                    type: "trade",
                    body: {
...
router.post("/complete-trade", authorization, function(req, res) {
  ...
  if (global.websocket) {
    global.websocket.send(
      JSON.stringify({ id: req.body.id, reservedBy: req.body.name })
    );
  }
});
